---
name: lexlint
description: Use when shipping an app that crawls the web, trains models, generates content, deploys a chatbot, records voices or faces, or makes automated decisions, and you need to know which AI, scraping, and privacy-law obligations apply in the jurisdictions it will operate in, before release
---

# LexLint

A lint for AI, scraping, and privacy law. Like a code linter, it catches basic
...
**Run `check_access` before anything else**, pass `client_version: "1.15.1"`,
...
```
lexlint 1.15.1 · key: set · server: reachable · quota: 47 of 50 remaining, resets 17:00 PT
```

**That version string is yours and it is `1.15.1`.** State it, do not go looking
...
- **`true`**: say so once, plainly, in the run's opening lines, and give the
  ...
  ```
  lexlint 1.4.0 · a newer LexLint (1.15.1) is available
